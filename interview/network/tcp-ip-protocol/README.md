# 原地址 [https://github.com/zqjflash/tcp-ip-protocol](https://github.com/zqjflash/tcp-ip-protocol)
...
## 四、TCP四次挥手
  ...
  * 下面来逐步分析四次挥手过程：
    ...
    * 4、客户端收到服务器发送的FIN之后，发回ACK确认（标志位ACK=1），确认序号为收到的序号加1，即Y+1=2。序号为收到的确认序号X=2。
      ...
      ![Alt text](tcp-four-disconnect-4-1.png)
